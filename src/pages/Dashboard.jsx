...
// estilos simples para os cards usados na aba REPORT
const s = {
  card: {
...
export default function Dashboard() {
  ...
  const loadForecastsAndMetrics = async (siloId) => {
    ...
    try {
      ...
      // 3. Texto explicativo vindo do backend
      try {
  ...
  // Auto-load meteorology quando entra na aba report
  useEffect(() => {
    let mounted = true;
    const doLoad = async () => {
      try {
  ...
  // MÉTRICAS SIMPLES PARA O DASH PRINCIPAL (já existiam)
    ...
    <Container>
      <MainContent>
        <Tabs>
          ...
          {/* ... demais abas que você já tinha */}
        </Tabs>

        {/* === ABA RELATÓRIO COM OS 4 CARDS === */}
        {activeTab === "report" && (
          <div>
            ...
            <div style={s.card}>
              ...
              {/* Previsão meteorológica resumida (se quiser manter) */}
        ...
        {/* Aqui você mantém o resto das abas (dashboard principal, silos, alertas, etc.) */}
      </MainContent>
    </Container>
  );
}
