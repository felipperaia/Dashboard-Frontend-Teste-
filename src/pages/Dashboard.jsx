          ...
          <div>
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px,1fr))', gap:20}}>
              {[{
                ...
                <div key={metric.key} style={{...s.card, display:'flex', flexDirection:'column', alignItems:'flex-start'}}>
                  <div style={{display:'flex', alignItems:'center', gap:12}}>
                    <img src={metric.icon} alt={metric.title} style={{width:28,height:28}} />
                    <h4 style={{margin:0}}>{metric.title}</h4>
                  </div>
                  <div style={{marginTop:12, display:'flex', alignItems:'baseline', gap:8}}>
                    <span style={{fontSize:28, fontWeight:700}}>{metric.value}</span>
                    <span style={{color:'#6b7280'}}>{metric.unit}</span>
                  </div>
